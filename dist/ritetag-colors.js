...
(function() {
  ...
  ritetagcolors = (function() {
    ritetagcolors.prototype.selector = 'ritecolor';    

    ritetagcolors.prototype.api = 'https://ritetag.com/api/v2/ai/twitter/';

    ritetagcolors.prototype.CONSUMER_KEY = '';

    ritetagcolors.prototype.CONSUMER_SECRET = '';

    ritetagcolors.prototype.OAUTH_TOKEN = '';

    ritetagcolors.prototype.OAUTH_TOKEN_SECRET = '';
    
    ritetagcolors.prototype.getnonce = function() {
        var length = 32;
        var text = "";
    ...
    ritetagcolors.prototype.ritething = function (hashtag) {
       var req,
       newnonce = this.getnonce(),
       colors = ["Unused", "Overused", "Good", "Great"],       
       time = Math.floor(Date.now() /1000),
       httpMethod = 'GET',
        pathto = this.api+hashtag,
        parameters = {
        ...
        encodedSignature = oauthSignature.generate(httpMethod, pathto, parameters, consumerSecret, tokenSecret);
      ...
      req.onreadystatechange = function() {
        ...
        if (req.readyState === 4) {
          if (req.status === 200 || req.status === 304) {
            ...
            data = data.replace("#"+hashtag, "<a class='"+colors[res.queryHashtag.color]+"' href='https://ritetag.com/hashtag-stats/"+hashtag+"'>#"+hashtag+"</a>");
      ...
      req.open('GET', this.api+hashtag+"?s=1&oauth_consumer_key="+this.CONSUMER_KEY+"&oauth_token="+this.OAUTH_TOKEN+"&oauth_signature_method=HMAC-SHA1&oauth_signature="+encodedSignature+"&oauth_timestamp="+time+"&oauth_nonce="+newnonce+"&oauth_version=1.0" );
      req.send();
    };
    ritetagcolors.prototype.parseInput = function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        match = ref[i];        
        this.ritething(match.trim().replace('#', ''));
      }     
    };
